@testset "Integrate: 1-dim" begin
	...
	@test Set([block.weight for block in values(grid)]) == Set([1,2])
end
...
@testset "Integrate: 2-dim" begin
	...
	# integrate should not change block weights.
	@test Set([block.weight for block in values(grid)]) == Set([1,2,3,4])
end
...
@testset "Integral model: 1-dim" begin
	...
	# weights = [1,2] because of order: [left, right]



	@test weights == [1,2]

	# Approximate to correct for floating point errors.
...
@testset "Integral model: 2-dim" begin
	grid = named_2d_grid()
	# Order of blocks is [bottom_left, bottom_right, top_left, top_right] -> weights = [3,4,1,2]
	...
	# weights = [3,4,1,2] because of order [bottom_left, bottom_right, top_left, top_right].
